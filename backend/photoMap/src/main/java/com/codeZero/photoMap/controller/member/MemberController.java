...
public class MemberController {
    ...
    /**
     * 카카오 회원가입 & 로그인 API
     * @param accessToken 카카오API accessToken
     * @return ResponseEntity<ApiResponse<TokenResponse>> 로그인 성공 시 액세스 토큰 및 리프레시 토큰을 포함한 응답 DTO
     */
    ...
    public ResponseEntity<ApiResponse<TokenResponse>> kakaoLogin(
        ...
        //2. 사용자 정보로 로그인, 회원가입 처리
        TokenResponse tokenResponse = memberService.processKakaoLogin(userInfo.getEmail(), userInfo.getNickname());

        //3. Authorization 헤더에 access 토큰 추가
        HttpHeaders headers = new HttpHeaders();
        headers.add("Authorization", "Bearer " + tokenResponse.getAccessToken());

        //4. ApiResponse 형식으로 JSON 응답 생성
        ApiResponse<TokenResponse> response = ApiResponse.ok(tokenResponse);

        //5. 헤더(accessToken포함)와 ApiResponse 응답 반환
        return ResponseEntity.ok().headers(headers).body(response);
    }
